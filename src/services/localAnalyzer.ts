 ...
 */
function groupQodlo(qodloStrings: string[], timeline?: { day: number; date?: string; }[]) {
  ...
  for (const s of qodloStrings) {
    ...
    if (match) {
      ...
      // Normalize message for better grouping (remove specific day numbers)
      let normalizedMessage = message.replace(/hari ke-\d+/gi, "hari tersebut");
      
      let dateString: string | undefined = undefined;
      if (timeline) {
         const tlItem = timeline.find(t => t.day === day);
         if (tlItem) dateString = tlItem.date;
      }
      
      parsed.push({ day, message, normalizedMessage, date: dateString });
    } else {
      nonStandard.push(s);
    }
  }
  ...
  const groups: { startDay: number; endDay: number; startDate?: string; endDate?: string; message: string }[] = [];
  
  if (parsed.length > 0) {
    parsed.sort((a, b) => a.day - b.day);
    let current = { ...parsed[0], startDay: parsed[0].day, endDay: parsed[0].day, startDate: parsed[0].date, endDate: parsed[0].date };

    for (let i = 1; i < parsed.length; i++) {
      const item = parsed[i];
      if (item.day === current.endDay + 1 && item.normalizedMessage === current.normalizedMessage) {
        ...
        groups.push({
          ...
          message: current.startDay === current.endDay ? current.message : current.normalizedMessage
        });
        current = { ...item, startDay: item.day, endDay: item.day, startDate: item.date, endDate: item.date };
      }
    }
    groups.push({
      startDay: current.startDay,
...
function buildFinalNifasResult(
  ...
  return {
    ...
    groupedQadho: groupQodlo(qodloSholat, statusTimeline)
  };
}
...
function evaluateMubtadiahMumayyizah(days: DayStrength[], isFirstMonth: boolean, context: string): FiqhAnalysisResult {
    ...
    days.forEach((d, idx) => {
        ...
        if (isFirstMonth && !isHaid && d.dayNumber <= 15 && d.isBlood) {
            qadhoObligations.push(`Sholat hari ke-${d.dayNumber}: Wajib diqodho. Karena baru pertama kali istihadloh, Anda wajib mengqodho sholat pada masa yang dihukumi istihadloh.`);
        }
    });
    ...
    if (isFirstMonth) {
        let haidDateString = "";
        const haidDaysArr = Array.from(haidIndices).map(i => days[i]).sort((a,b) => a.dayNumber - b.dayNumber);
        if (haidDaysArr.length > 0) {
            const startDay = haidDaysArr[0];
            const endDay = haidDaysArr[haidDaysArr.length - 1];
            if (startDay.date && endDay.date) {
               haidDateString = ` (dari Tgl ${format(parseISO(startDay.date), 'd MMMM', { locale: id })} sampai ${format(parseISO(endDay.date), 'd MMMM', { locale: id })})`;
            }
        }
        specialNotes.push(`Catatan Khusus: Jika bulan berikutnya mengalami istihadloh lagi, Anda wajib mandi dan sholat mengikuti siklus haid saat ini${haidDateString}.`);
...
export function evaluateMubtadiahGhoiruMumayyizah(days: DayStrength[], isFirstMonth: boolean): FiqhAnalysisResult {
    ...
    if (isFirstMonth) {
        let dateString = "";
        let mandiDateString = "";
        const startDay = days.find(d => d.dayNumber === 2);
        const endDay = days.find(d => d.dayNumber === 15) || days[days.length - 1]; // Fallback to last day
        
        if (startDay && startDay.date && endDay && endDay.date) {
            dateString = ` (dari Tgl ${format(parseISO(startDay.date), 'd MMMM', { locale: id })} sampai ${format(parseISO(endDay.date), 'd MMMM', { locale: id })})`;
            const mandiDate = addDays(parseISO(endDay.date), 1);
            mandiDateString = `, lalu Anda wajib mandi besar dan mulai sholat kembali pada Tgl ${format(mandiDate, 'd MMMM', { locale: id })}`;
        }
        let haidDateString = "";
        if (days.length > 0 && days[0].date) {
             haidDateString = ` (pada Tgl ${format(parseISO(days[0].date), 'd MMMM', { locale: id })})`;
        }
        specialNotes.push(`Catatan Bulan Pertama: Karena baru pertama kali istihadloh, Anda wajib mengqodho sholat pada masa yang dihukumi istihadloh${dateString}${mandiDateString}. Jika bulan berikutnya mengalami istihadloh lagi, Anda wajib mandi dan sholat mengikuti siklus haid saat ini${haidDateString}.`);
    } else {
...
export function evaluateMutadahMumayyizah(days: DayStrength[], habit: UserHabit, isFirstMonth: boolean): FiqhAnalysisResult {
    ...
    if (isFirstMonth) {
        let dateString = "";
        let mandiDateString = "";
        // Untuk Golongan 3, masa menanti adalah masa lemah dalam 15 hari pertama
        const qadhoDays = days.filter(d => !d.isStrong && d.dayNumber <= 15);
        if (qadhoDays.length > 0) {
            const startDay = qadhoDays[0];
            const endDay = qadhoDays[qadhoDays.length - 1];
            if (startDay.date && endDay.date) {
               dateString = ` (dari Tgl ${format(parseISO(startDay.date), 'd MMMM', { locale: id })} sampai ${format(parseISO(endDay.date), 'd MMMM', { locale: id })})`;
               const mandiDate = addDays(parseISO(endDay.date), 1);
               mandiDateString = `, lalu Anda wajib mandi besar dan mulai sholat kembali pada Tgl ${format(mandiDate, 'd MMMM', { locale: id })}`;
        ...
        specialNotes.push(`Catatan Bulan Pertama: Karena baru pertama kali istihadloh, Anda wajib mengqodho sholat pada masa yang dihukumi istihadloh${dateString}${mandiDateString}. Jika bulan berikutnya mengalami istihadloh lagi, Anda wajib mandi dan sholat mengikuti siklus haid saat ini.`);
    }
        ...
        if (weakSession.days.length >= 15) {
            ...
            days.forEach((d, idx) => {
                ...
                if (isFirstMonth && status === 'Istihadloh' && d.dayNumber <= 15) {
                  if (d.isBlood) {
                    if (isWaiting) {
                      qadhoObligations.push(`Sholat hari ke-${d.dayNumber}: Wajib diqodho. Karena baru pertama kali istihadloh, Anda wajib mengqodho sholat pada masa yang dihukumi istihadloh. Jika bulan berikutnya mengalami istihadloh lagi, Anda wajib mandi dan sholat mengikuti siklus haid saat ini.`);
                    } else {
                      qadhoObligations.push(`Sholat hari ke-${d.dayNumber}: Wajib diqodlo (Kasus Darah Istihadloh). Anda meninggalkan sholat pada saat darah keluar yang ternyata dihukumi Istihadloh.`);
                    }
                  } else {
...
export function evaluateMutadahGhoiruMumayyizahDzakiroh(days: DayStrength[], habit: UserHabit, calculationMonthIndex: number = 0, isFirstMonth: boolean = false): FiqhAnalysisResult {
    ...
    if (isFirstMonth) {
        ...
        let mandiDateString = "";
        const durAdat = habit.duration || 7;
        const qadhoDays = days.filter(d => d.dayNumber > durAdat && d.dayNumber <= 15);
        if (qadhoDays.length > 0) {
            const startDay = qadhoDays[0];
            const endDay = qadhoDays[qadhoDays.length - 1];
            if (startDay.date && endDay.date) {
...
function getCategoryReason(shortCategory: string, category: string, hasGaps: boolean = false): string {
  ...
  return {
    ...
    groupedQadho: groupQodlo(cleanQadho, finalResult.statusTimeline)
  };
}
